...
void loop() {
  ...
  //colorWipe(strip.Color(127,   50,   50), 50);  // Red
  //colorWipe(strip.Color(  50, 127,   50), 50);  // Green
  //colorWipe(strip.Color(  1,   0, 0), 2);  // Blue
  //colorWipe(strip.Color(  1,   1, 0), 2);
  //colorWipe(strip.Color(  0,   0, 0), 5);  // Blue
...
void bloques(uint8_t wait) {
  ...
  for(i=0; i < strip.numPixels(); i++){
    ...
    if(random(3)==0){
      strip.setPixelColor(random(strip.numPixels()), strip.Color(1,   1, 0));
    }
    strip.show();
  }
